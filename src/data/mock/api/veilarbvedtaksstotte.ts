...
import { DEFAULT_DELAY_MILLISECONDS, fellesMockResponseHeaders } from './index';
import {
    ArenaKode,
    GammelInnsatsgruppe,
    Hovedmal,
...
export const veilarbvedtaksstotteHandlers: RequestHandler[] = [
    http.post('/veilarbvedtaksstotte/api/hent-gjeldende-14a-vedtak', async () => {
        ...
        return HttpResponse.json(gjeldende14aVekdak, { headers: { ...fellesMockResponseHeaders } });
    }),
    http.get('/veilarbvedtaksstotte/open/api/v2/kodeverk/innsatsgruppeoghovedmal', async () => {
        await delay(DEFAULT_DELAY_MILLISECONDS);
        return HttpResponse.json(kodeverkdata, { headers: { ...fellesMockResponseHeaders } });
    })
];
